# group by
# 특정 칼럼을 기준으로 그룹핑하여 그룹함수를 사용해 원하는 데이터를 추출할 목적으로 사용
# group 함수 : sum(). count(), avg(), max(), min()
# select 절에는 그룹핑한 기준이 되는 컬럼과, 그 외 함수의 결과만 올 수 있다.

# 부서별 급여의 총액과 평균 급여를 구해보자
...
# 급여가 300만 원 이상인 사원들의 부서별 급여 총액을 구하시오.
select dept_code, sum(salary)
from employee
where salary >= 3000000
group by dept_code;

# 부서의 평균 급여가 3000000이상인 부서를 조회하시오.
select dept_code
from employee
group by dept_code
having sum(salary) >= 3000000;

# 부서 내에 , 직급별 급여의 최소 최대값을 구해보자.
select dept_code, job_code, max(salary), min(salary)
from employee
