...
select * from emp_dept;
#박명수 사원의 부서를 유재석 사원과 동일하게 변경시켜주세요
set dept_title a  = (select dept_title b from emp_dept where emp_name = '대북혼') 
...
# rollback : 테이블을 이전 commit 상태로 돌림

# a계좌 --- 5만원 ---> b계좌

# a계좌에 잔금을 5만원 차감한 금액으로 update
# b계좌에 잔금을 5만원 증액한 금액으로 update(문제발생!)
